...
func GetOrderConsumer(config *MQConfig) (*OrderConsumer, error) {
...
func (p *OrderConsumer) BindQueue(QueueName, Exchange, RoutingKey string) error {
	...
	if err != nil {
		klog.Errorf("队列绑定失败: %v", err)
		return fmt.Errorf("队列绑定失败: %w", err)
	}
	return nil
}

// Consume 消费者指定一条队列消费信息
func (p *OrderConsumer) Consume(ctx context.Context, msgHandler MessageHandler) error {
	retryCount := 0
...
// 消息处理封装
func (p *OrderConsumer) handleMessage(h MessageHandler, msg amqp.Delivery) error {
	defer func() {
		if r := recover(); r != nil {
...
// 消息处理封装
func (p *OrderConsumer) reconnectIfNeeded() error {
	// 检查连接状态
	if p.mq.Conn == nil || p.mq.Conn.IsClosed() {
		newConn, err := amqp.Dial(conf.GetConf().RabbitMQ.MqURL)
		if err != nil {
